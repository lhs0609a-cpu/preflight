...
function profileSlugs() {
  const dir = path.join(root, 'packages/profiles')
  try {
    return readdirSync(dir)
      .filter((f) => f.endsWith('.json') && f !== 'profile.schema.json')
...
const preflight = {
  rules: {
    'no-profile-slug-literal': {
      meta: {
        type: 'problem',
        docs: { description: '거래 유형 슬러그를 코드에 하드코딩하지 않는다 (07 합격 기준 2)' },
        schema: [],
        messages: {
          slug: '프로파일 슬러그 "{{slug}}" 가 코드에 있다. 분기 대신 프로파일 데이터로 표현하라.',
        },
      },
      create(context) {
        ...
        return {
          Literal(node) {
            if (typeof node.value === 'string' && SLUGS.has(node.value)) {
              context.report({ node, messageId: 'slug', data: { slug: node.value } })
            }
          },
        }
      },
...
export default tseslint.config(
  ...
  ...tseslint.configs.recommended,
  {
    plugins: { preflight },
    rules: {
      'preflight/no-profile-slug-literal': 'error',
